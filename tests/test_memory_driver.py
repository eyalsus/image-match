import pytest
from numpy import ndarray, array_equal
try:
    from urllib.request import urlretrieve
except:
    from urllib import urlretrieve

from image_match.memory_driver import SignatureMemory

test_img_url = 'https://camo.githubusercontent.'
test_diff_img_url = 'https://camo.githubusercontent.com/826e23bc3eca041110a5af467671b012606aa406/68747470733a2f2f63322e737461746963666c69636b722e636f6d2f382f373135382f363831343434343939315f303864383264653537655f7a2e6a7067'
urlretrieve(test_img_url, 'test.jpg')

@pytest.fixture
def smem():
    return SignatureMemory()

def test_index_refresh(smem):
    smem.add_image('test.jpg')
    r = smem.search_image('test.jpg')
    assert len(r) == 1 and r[0]['dist'] == 0

def test_image_search(smem):
    smem.add_image('test.jpg')
    r = smem.search_image(test_diff_img_url)
    assert len(r) == 1 and 0.42 < r[0]['dist'] < 0.43